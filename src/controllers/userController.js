...
export async function getUsersWithActivePlans(req, res) {
    ...
    try {
        ...
        console.log(`Fetching users with active plans (page: ${page}, limit: ${limit})`);
        ...
        const query = `
            SELECT 
                ...
                COALESCE(json_agg(
                    ...
                    ELSE NULL
                    END
                ) FILTER (WHERE us.id IS NOT NULL AND us.status = 'active'), '[]') AS active_plans
            FROM 
                public.users u
            LEFT JOIN 
                user_subscriptions us 
                ON u.id = us.user_id AND us.status = 'active' AND us.ends_at > CURRENT_TIMESTAMP
            LEFT JOIN 
...
export async function getUserSubscriptionHistory(req, res) {
    ...
    try {
        ...
        console.log(`Retrieved ${result.rows.length} subscription records for user ID: ${userId}`);
